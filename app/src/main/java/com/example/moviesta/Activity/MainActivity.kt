...
import com.example.moviesta.BottomNavigationBar
import com.example.moviesta.FilmItem
import com.example.moviesta.R
import com.example.moviesta.SearchBar
import com.example.moviesta.ViewModel.MainViewModel
...
fun MainScreen(onItemClick:(FilmItemModel) ->Unit={}) {
    Scaffold(
        ...
        floatingActionButton = {
           ...
           backgroundColor = colorResource(R.color.BlackBacground)
    ) { paddingValues ->
        Box(
...
val viewModel = MainViewModel()
    val upcoming = remember { mutableListOf<FilmItemModel>() }
    val newMovies = remember { mutableListOf<FilmItemModel>() }
    ...
    LaunchedEffect(Unit){
        viewModel.loadUpcoming().observeForever(){
            Log.d("MainContent", "Upcoming items: ${it.size} — $it")
            upcoming.clear()
            upcoming.addAll(it)
